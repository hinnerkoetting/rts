...
ButtonRessource::ButtonRessource(float x1, float x2, float y1, float y2, float relX, float relY, int id, int (*getWish)(int, int), int (*getCurrent)(int, int), void (*inc)(int, int, int ),int party, int village) {
	this->x1 = x1;
	this->x2 = x2;
	this->y1 = y1;
	this->y2 = y2;
...
ButtonRessource::ButtonRessource(float x1, float x2, float y1, float y2, float relX, float relY, int id, BtnRessourceHandler* handl, GameObject::gameObject_ID ch_Id) {
	this->x1 = x1;
	this->x2 = x2;
	this->y1 = y1;
	this->y2 = y2;
 ...
 *
 */
void ButtonRessource::click(int button) {
	if (button == GLUT_LEFT_BUTTON){
		handler->changeNrWorker(changeID,1);
	}
