...
void Unit::move() {
	if (abs(this->getExactX() - (int)(getPosConst() * pf->getNextField()->x)) < posTolerance()  
		...
		if (pf->getPath().size() == 1) {				// unit has no further way
			...
			//worldModel::fields[pf->getNextField()->x, pf->getNextField()->y]->setBlocked(this);
			return;
		}
		else { // unit has further way
			this->pf->checkPath(this->getX(), this->getY()); // check next two worldModel::fields if barriers which existed at beginning of movement do still exist
			...
			if (actual->x >0 && actual->x < Options::iNumberEdgesX && actual->y >0 && actual->y < Options::iNumberEdgesY) // field does really exist
				if (next->x >0 && next->x < Options::iNumberEdgesX && next->y >0 && next->y < Options::iNumberEdgesY)
					if (!worldModel::fields[next->x][next->y].blocked()) { // if next field is not blocked
						worldModel::fields[actual->x][actual->y].setBlocked(0);  //unblock field where unit just left
						pf->nextField();									// retrieve next field where this unit is supposed to go to
						worldModel::fields[next->x][next->y].setBlocked(this);	// block that field
					}
...
bool Unit::nextToBuilding(int buildingID) {
	...
	GameObject* g;
	if (y > 0) {
		g = worldModel::fields[x][y-1].blockedBy();
		if (g != 0)
			if (g->getID() == buildingID)
				return true;
		if (x > 0) {
			g = worldModel::fields[x-1][y-1].blockedBy();
			if (g != 0)
				if (g->getID() == buildingID)
					return true;
		}
	...
	if (x > 0) {
		GameObject* g = worldModel::fields[x-1][y].blockedBy();
		if (g != 0)
			if (g->getID() == buildingID)
				return true;
	}
